...
if __name__ == '__main__':
	...
	# optimal slope and intercept values determined in earlier part of the PSET
	m_opt = 62/35
	b_opt = 18/35
	...
	X_space = []
	y_space = []
	X_space = np.linspace(0, 5, num=100)
	y_space = np.array([b_opt + (m_opt * x) for x in X_space]).reshape(-1,1)
	...
	y_space_rand = np.array([noise[i] + b_opt + (m_opt * X_space[i]) for i in range(len(X_space))]).reshape(-1,1)
	...
	half_ones = ones[0:int(len(ones)/2)]
	first_half_X = X_space[0:int(len(X_space)/2)]
	second_half_X = X_space[int(len(X_space)/2):len(X_space)]

	X_space_stacked = np.hstack(([half_ones,first_half_X],[half_ones,second_half_X]))

	XT = X_space_stacked
	XC = X_space_stacked.transpose()
	XT_XC = np.matmul(XT,XC)
	XT_Y = np.matmul(XT,y_space_rand)
	W_opt = np.linalg.solve(XT_XC,XT_Y)
	...
	y_pred_rand = np.array([b_rand_opt + (m_rand_opt * i) for i in X_space]).reshape(-1,1)
